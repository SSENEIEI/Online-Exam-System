...
function extractJsonFromResponse($apiResponse) {
    if (isset($apiResponse['candidates'][0]['content']['parts'][0]['text'])) {
        ...
        // Clean the text to get only the JSON part
...
$mcqCount = $requestBody['mcq_count'] ?? 0;
$writtenCount = $requestBody['written_count'] ?? 0;
...
if ($examType === 'multiple_choice') {
    $typeInstruction = " (Important: All questions MUST be of type 'multiple_choice'.)";
} elseif ($examType === 'written') {
    $typeInstruction = " (Important: All questions MUST be of type 'written'.)";
