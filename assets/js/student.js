...
function toggleMixedOptions() {
  ...
  if (examTypeEl.value === 'mixed') mixed.classList.remove('hidden'); else mixed.classList.add('hidden');
}
...
async function handlePracticePrompt(isCorrection = false) {
  ...
  try {
    ...
    const writtenCount = document.getElementById('written-count')?.value || '';
...
function initAntiCheatSystem() {
  ...
  document.addEventListener('contextmenu', e => { e.preventDefault(); antiCheatData.rightClicks++; showWarning('ห้ามคลิกขวา!'); });
  document.addEventListener('visibilitychange', () => { if (document.hidden) { antiCheatData.tabSwitches++; showWarning('ห้ามเปลี่ยนแท็บ!'); } });
  window.addEventListener('blur', () => { antiCheatData.tabSwitches++; });
  document.addEventListener('keydown', e => {
    if (e.key==='F12'||(e.ctrlKey&&e.shiftKey&&['I','C','J'].includes(e.key))||(e.ctrlKey&&['U','S','A','P','F','R'].includes(e.key))||e.key==='F5'||(e.altKey&&e.key==='Tab')) { e.preventDefault(); antiCheatData.keyboardShortcuts++; showWarning('บล็อกคีย์ลัด!'); }
  });
}
function showWarning(msg){ const o=document.createElement('div'); o.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(220,53,69,.9);z-index:10000;display:flex;align-items:center;justify-content:center;color:#fff;font-size:24px;font-weight:bold;text-align:center;padding:20px;'; o.innerHTML=`<div><div style='font-size:48px;margin-bottom:20px;'>⚠️</div><div>${msg}</div><div style='font-size:16px;margin-top:20px;opacity:.8;'>บันทึกแล้ว</div></div>`; document.body.appendChild(o); setTimeout(()=>o.remove(),3000);} 
function getAntiCheatReport(){ return { tabSwitches:antiCheatData.tabSwitches,rightClicks:antiCheatData.rightClicks,keyboardShortcuts:antiCheatData.keyboardShortcuts,fullscreenExits:antiCheatData.fullscreenExits,examDuration:antiCheatData.startTime?Date.now()-antiCheatData.startTime:0,suspicious:antiCheatData.suspicious,logs:JSON.parse(sessionStorage.getItem('antiCheatLogs')||'[]') }; }
